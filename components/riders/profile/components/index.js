...
import { updateRider, loadRider, GET_RIDER_FULFILLED, PUT_RIDER_FULFILLED } from '../actions'
...
class Profile extends Component {
    ...
    async componentDidMount() {
        ...
        this.props.onLoadRider(userData.id).then(res => {
            if (res.type === GET_RIDER_FULFILLED) {
    ...
    async handleUpdate() {
        ...
        if (errorMessage.length === 0) {
            if (!this.state.imageUri) {
                this.props.onUpdateRider(submitData).then(res => {
                    this.setState({ isLoading: false })
                    if (res.type === PUT_RIDER_FULFILLED) {
                        Alert.alert(
                            "Updated",
                ...
                RNS3.put(this.state.image, S3Config('users')).then(response => {
                    ...
                    this.props.onUpdateRider({ ...submitData, image: `users/${this.state.image.name}` }).then(res => {
    ...
    render() {
        console.log("`${REACT_APP_STORAGE}/${this.state.image}`", `${REACT_APP_STORAGE}/${this.state.image}`)
        return (
                    ...
                    <Card >
                        <View style={{ marginTop: 10 }}  >
                            <Button
                                ...
                                title="UPDATE"
                                type="solid"
                                loading={this.props.riderLoading || this.state.isLoading}
                                onPress={() => {
                                    this.setState({ isLoading: true })
                                    this.handleUpdate();
                                }}
...
function mapStateToProps(state) {
    return {
        riderSuccess: state.riders.profile.riderSuccess,
        riderLoading: state.riders.profile.riderLoading,
    };
}
