...
export const POST_ACCEPT_RIDER = 'POST_ACCEPT_RIDER'
export const POST_ACCEPT_RIDER_FULFILLED = 'POST_ACCEPT_RIDER_FULFILLED'
export const POST_ACCEPT_RIDER_REJECTED = 'POST_ACCEPT_RIDER_REJECTED'
...
export const acceptRider = (riderId, transactionId, serviceFee) => {
    const submitData = {
        transactionId,
        serviceFee
    }
    return async dispatch => {
        dispatch({
            type: POST_ACCEPT_RIDER,
            payload: {}
        })
        const token = await getSessionToken()
        return Axios.post(`${config.REACT_APP_API_BASE_URL}/rider-accepted/${riderId}`, { ...submitData }, {
            headers: {
                Accept: 'application/json;charset=UTF-8',
                'Content-Type': 'application/json;charset=UTF-8',
                Authorization: `Bearer ${token}`
            ...
            return dispatch({
                type: POST_ACCEPT_RIDER_FULFILLED,
                payload: response.data
            })
        }).catch((error) => {
            console.log('err', error)
            return dispatch({
                type: POST_ACCEPT_RIDER_REJECTED,
                payload: error
            })
        })
    }
